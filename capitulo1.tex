...
Actualmente la existencia, competitividad y rentabilidad de diversas compañías depende de los flujos de datos. Sin embargo, la variedad de fuentes, tipos y volúmenes de datos ha vuelto más complejo el proceso de encontrarlos \citep{AloomaInc.2017}. Para afrontar esta tarea existen herramientas informáticas conocidas como ``Herramientas de Extracción, Transformación y Carga'' (``Extract, Transform, and Load'' - ETL, por sus siglas de acuerdo al término en idioma inglés). Los principales propósitos de una herramienta ETL según \cite{AloomaInc.2017} son:

\begin{itemize}
\item Extraer datos de bases de datos legadas (usualmente heterogéneas).
\item Transformar los datos (de datos optimizados para transacciones a datos optimizados para el análisis).
\item Sincronización y limpieza de los datos.
\item Carga de los datos en un almacén de datos.
\end{itemize}

Con el propósito de identificar posibles herramientas ETL que permitiesen realizar una integración de datos sin despreciar la semántica de los mismos se analizó una muestra definida por \cite{AloomaInc.2017}. El criterio de comparación que se escogió fue el mecanismo para la descripción de los datos.
